...
<!--        row为横向布局-->
        <el-row style="flex: 4;min-width: 310px;">
           ...
           border-bottom:1px solid  #dadada;">
            ...
            <el-col style="width: 100%;height: 90%;padding: 15px 0 0 25px" class="row1">
                <el-tree
                        v-loading="loading"
        ...
        <el-row style="flex: 4;min-width: 310px;padding: 25px 0 0 25px" class="row2">
            <el-col style="padding: 25px 0 15px 0;
            position:relative;font-size:15px;
            width: 100%;height: 10%;
            min-height:45px;
           border-bottom:1px solid  #dadada;">
            ...
            <el-col style="width: 100%;height: 90%;padding: 15px 0 0 25px">
            <el-tree
                    v-loading="loading"
                    element-loading-text="拼命加载中"
                    element-loading-spinner="el-icon-loading"
                ...
                return new Promise((resolve)=> {
                    this.$http.get('organddevicenode/getallbinds', {}).then(response => {
                        if (response.body.status === 200) {
                            this.bind = response.body.result;
                            resolve(this.bind);
                        }
                    })
                })
